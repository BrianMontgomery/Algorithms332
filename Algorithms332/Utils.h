...
inline std::ifstream open_cmdline_file(int argc, const char* argv[], size_t args_desired, const std::string& usage_msg, bool exact_args = true) {
...
// printing utilities
//==========================================================================
inline std::string yes_or_no(bool condition) { return condition ? "YES" : "no"; }

template <typename T>
inline void print(const std::string& msg, T* arr, size_t low, size_t high) {
	std::cout << msg;
	for (size_t i = low; i < high; ++i) { std::cout << arr[i] << " "; }
}

template <typename T>
inline void print(const std::string& msg, T* arr, size_t n) { print(msg, arr, 0, n - 1); }


inline char* strconvert(char* s, int(*fp)(int)) {
	char* p = s;
	while (*p != '\0') { *p = fp(*p);  ++p; }
	return s;
}
...
template <typename T>
inline bool less(T& v, T& w, const comparator<T>& comp) {
	bool value = comp(v, w);
	return value;
}
...
template <typename T>
inline int compare(T& v, T& w, const comparator<T>& comp) {
	int result;

	if (less(v, w, comp)) {
		result = -1;
	}
	else if (less(w, v, comp)) {
		result = 1;
	}
	else { result = 0; }
